...
adb("install", "-r", "release/HellRun-Android.apk")
adb("logcat", "-c")
...
for attempt in range(15):
    time.sleep(1)
    tree = ui()
    start = find(tree, ["BOSHLASH"])
    if start: break
assert start, "Start button did not render in the Android WebView"
(OUT / "android-menu.png").write_bytes(adb("exec-out", "screencap", "-p"))
tap(start)
time.sleep(1)
(OUT / "android-game.png").write_bytes(adb("exec-out", "screencap", "-p"))
pause = find(ui(), ["PAUZA", "Pauza"])
assert pause, "Pause control missing after starting"
tap(pause)
time.sleep(.4)
assert find(ui(), ["DAVOM ETISH"]), "Pause overlay did not appear"
adb("shell", "input", "keyevent", "3")
adb("shell", "am", "start", "-W", "-n", APP + "/.MainActivity")
time.sleep(1)
assert find(ui(), ["DAVOM ETISH"]), "Pause state lost across background/resume"
