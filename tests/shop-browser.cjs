...
module.exports=async function checkShop(browser,url,watch,menuFits){
 ...
 await page.evaluate(()=>{game.progress.wallet=100505;game.save();showSkins(true);});await page.locator('[data-skin="sunforged"]').tap();await page.locator('#skinActionBtn').tap();
 ...
 assert.deepEqual(await page.evaluate(()=>[game.progress.wallet,game.progress.equippedSkin,game.progress.ownedSkins]),[505,'sunforged',['default','sunforged']]);assert.equal(await page.locator('#skinActionBtn').isDisabled(),true);
 await page.screenshot({path:'verification/skin-shop-phone.png'});
 await page.reload();await page.locator('#skinShopBtn').tap();assert.equal(await page.evaluate(()=>game.progress.equippedSkin),'sunforged');await page.locator('[data-rank="owned"]').tap();assert.equal(await page.locator('[data-skin]').count(),2);
 await page.locator('[data-skin="default"]').tap();await page.locator('#skinActionBtn').tap();assert.equal(await page.evaluate(()=>game.progress.equippedSkin),'default');await page.locator('[data-skin="sunforged"]').tap();await page.locator('#skinActionBtn').tap();assert.equal(await page.evaluate(()=>game.progress.wallet),505);
 await page.locator('#shopBackBtn').tap();await page.locator('#startBtn').tap();await page.waitForFunction(()=>game.state==='playing');
 assert.equal(await page.evaluate(()=>game.player.w),24);assert.equal(await page.evaluate(()=>game.player.h),32);await page.screenshot({path:'verification/sunforged-in-game.png'});
 ...
 await page.evaluate(()=>{I18n.set('uz');applyLocale();skinFilter='SS+';selectedSkin='stormmantle';showSkins(true);});await page.setViewportSize({width:844,height:390});await page.screenshot({path:'verification/skin-shop-landscape.png'});
