...
test('catalog has 40 paid skins, original legendary designs and eight price ranks',()=>{
 assert.equal(Skins.catalog.length,41);assert.equal(new Set(Skins.catalog.map(s=>s.id)).size,41);
 assert.deepEqual(Skins.catalog.filter(s=>s.rank==='SS+').map(s=>s.name),['Sunforged','Void Regent','Frostcrown','Clockwarden','Stormmantle','Emberdrake','Briarheart','Tidemantle','Prismarch','Obsidian','Starweaver','Lightkeeper']);
...
test('purchase debits once, equips immediately and survives reload with checkpoint and zero lives intact',()=>{
 ...
 const before=clone(g.progress);assert.equal(g.buySkin('sunforged'),true);assert.equal(g.buySkin('sunforged'),false);assert.equal(g.progress.wallet,5);assert.equal(writes.length,1);
 const restored=new Engine(writes[0]);assert.equal(restored.progress.equippedSkin,'sunforged');assert.deepEqual(restored.progress.ownedSkins,['default','sunforged']);assert.equal(restored.progress.lives,0);assert.equal(restored.run.checkpoint,4);assert.deepEqual(restored.run.collected,[1,2,3]);assert.deepEqual(restored.progress.completed,before.completed);
 restored.start();assert.equal(restored.state,'gameover');assert.equal(restored.buyLife(),true);assert.equal(restored.progress.wallet,0);assert.equal(restored.progress.equippedSkin,'sunforged');assert.equal(restored.run.checkpoint,4);
});
test('insufficient balance, unknown IDs and unowned equipment cannot change a save',()=>{
 ...
 for(const id of ['rookie','sunforged','missing','__proto__','constructor',null]){assert.equal(g.buySkin(id),false);assert.equal(g.equipSkin(id),false);}
...
test('shop operations require frozen play and never alter level physics or refill lives',()=>{
 const g=new Engine({...freshProgress(),wallet:100000});g.start();assert.equal(g.buySkin('sunforged'),false);assert.equal(g.equipSkin('default'),false);g.pause();assert.equal(g.buySkin('sunforged'),true);assert.equal(g.state,'paused');assert.equal(g.progress.lives,3);
...
test('upgrading v2/v3 preserves progress and initializes only the free skin',()=>{
 const raw={...freshProgress(),version:3,wallet:73000,lives:1,unlocked:24,currentLevel:17,completed:[1,2,3],ownedSkins:['sunforged'],equippedSkin:'sunforged'};
...
test('invalid or duplicate inventory entries are removed and unavailable equipment falls back',()=>{
 const p=normalizeProgress({...freshProgress(),ownedSkins:['sunforged','sunforged','bogus','__proto__',{},null],equippedSkin:'stormmantle'});assert.deepEqual(p.ownedSkins,['default','sunforged']);assert.equal(p.equippedSkin,'default');
});
